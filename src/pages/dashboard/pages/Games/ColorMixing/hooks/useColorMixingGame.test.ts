...
import { TARGET_COLORS } from '../utils/ryb'
...
describe('useColorMixingGame — easy mode', () => {
  ...
  it('nextRound advances round counter and picks new target in easy mode', () => {
    const { result } = renderHook(() => useColorMixingGame())
    act(() => result.current.actions.selectLevel('easy'))
    const firstTarget = result.current.state.targetColor
    act(() => result.current.actions.confirmMix())
    act(() => result.current.actions.nextRound())
    expect(result.current.state.phase).toBe('playing')
    expect(result.current.state.round).toBe(2)
    expect(result.current.state.mix).toEqual({ r: 0, y: 0, b: 0 })
    // Target should change (not guaranteed but overwhelmingly likely with 30 colors)
    expect(result.current.state.targetColor).toBeDefined()
    expect(typeof result.current.state.targetColor.name).toBe('string')
...
describe('useColorMixingGame — medium mode', () => {
  ...
  it('timeLeft decrements each second', () => {
    ...
    expect(result.current.state.timeLeft).toBe(57)
  })
